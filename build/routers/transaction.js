...
const router = Router(); // Expressルーターインスタンスを作成
// 日付整形用のヘルパー関数
...
router.put('/:id', async (req, res) => {
    ...
    try {
        ...
        if (error instanceof Error && error.code === 'P2025') {
            return res.status(404).json({ error: 'Transaction not found' }); // テストの期待値に合わせる
        }
...
router.delete('/:id', async (req, res) => {
    ...
    try {
        ...
        if (error instanceof Error && error.code === 'P2025') {
            return res.status(404).json({ error: 'Not found or 削除に失敗しました' }); // テストの期待値に合わせる
        }
